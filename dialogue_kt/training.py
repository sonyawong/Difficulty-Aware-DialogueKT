...
from pykt.models.dkt import DKT  # Lazy import
from pykt.models.akt import AKT  # Lazy import
from pykt.models.dkvmn import DKVMN  # Lazy import
from pykt.models.saint import SAINT  # Lazy import
from pyBKT.models import Model as BKT  # Lazy import
        ...
        if wandb is None:
            ...
            if api_key:
                try:
                    ...
                    wandb.init(
                        project=getattr(args, 'wandb_project', 'dialogue-kt'),
                        config=vars(args),
                    )
                except (ValueError, Exception) as e:
                    print(f"Warning: wandb login failed ({e}). Training will continue without wandb logging.")
                    print("To fix: get your 40-char API key from https://wandb.ai/authorize and save to ~/.wandb_api_key")
            else:
                print("Warning: WANDB_API_KEY not set. Training will continue without wandb logging.")
...
def train(args):
    ...
    if args.crossval:
        ...
        return fn(args, args.fold if getattr(args, "fold", None) else get_default_fold(args))
...
def compute_metrics(labels, preds):
    ...
    if len(unique_labels) < 2:
        print(f"⚠️  WARNING: Only one class present in labels: {unique_labels}. AUC cannot be computed.")
        auc = float('nan')  # AUC is undefined
    else:
        auc = roc_auc_score(labels, preds)

    prec, rec, f1, _ = precision_recall_fscore_support(labels, hard_preds, average="binary", zero_division=0)
    return acc * 100, auc * 100 if not np.isnan(auc) else auc, prec * 100, rec * 100, f1 * 100
...
def get_lmkt_loss_irt(model, batch, true_token, false_token, args):
    ...
    if hasattr(model, 'base_model'):
        model_dtype = getattr(model.base_model, 'dtype',
                     getattr(getattr(model.base_model, 'model', None), 'dtype', torch.float32))
    else:
    ...
    attention_mask[attention_mask == 0] = min_dtype
    attention_mask[attention_mask == 1] = 0
    attention_mask = attention_mask.type(model_dtype)
    ...
    batch_size = model_output.logits.shape[0]
    all_logits = model_output.logits  # [B, seq_len, vocab_size]
    dev = all_logits.device

    ability_pos = batch["level_last_idxs"][:, 0]
    difficulty_pos = batch["level_last_idxs"][:, 1]

    arange = torch.arange(batch_size, device=dev)
    ability_logits = all_logits[arange, ability_pos][:, batch["ability_token_ids"]]       # [B, 2]: GOOD, BAD
    difficulty_logits = all_logits[arange, difficulty_pos][:, batch["difficulty_token_ids"]]  # [B, 2]: HARD, EASY

    theta = ability_logits[:, 0] - ability_logits[:, 1]
    difficulty = difficulty_logits[:, 0] - difficulty_logits[:, 1]

    corr_probs = torch.sigmoid(model.logit_scale * (theta - difficulty))
    loss = torch.nn.BCELoss()(corr_probs.float(), batch["labels"].float())

    ability_classes = ability_logits.argmax(dim=1)
    difficulty_classes = difficulty_logits.argmax(dim=1)
    theta_grouped = [[theta[i].item(), difficulty[i].item(), ability_classes[i].item(), difficulty_classes[i].item()]
                     for i in range(batch_size)]

    return loss, theta_grouped, corr_probs

def train_lmkt(args, fold):
    use_irt = getattr(args, 'use_irt', False)

    model, tokenizer = get_model(
        args.base_model, False,
    ...
    get_loss = get_lmkt_loss_irt if use_irt else (get_lmkt_loss_packed if args.pack_kcs else get_lmkt_loss_unpacked)
    ...
    kc_dict = load_kc_dict(args) if args.pack_kcs else None
    collator = KTCollator(tokenizer, use_irt=use_irt, kc_dict=kc_dict) if args.pack_kcs else KTCollator(tokenizer)
    ...
    if args.optim == "adamw":
        ...
        optimizer = transformers.Adafactor(param_groups, lr=args.lr, weight_decay=args.wd, relative_step=False)

    # Optional warmup + cosine LR decay
    ...
    if lr_schedule == "cosine":
        # Total optimization steps (after gradient accumulation)
        num_update_steps_per_epoch = max(1, math.ceil(len(train_dataloader) / args.grad_accum_steps))
        ...
        print(f"Using cosine LR schedule with warmup_steps={warmup_steps}, total_steps={num_training_steps}")
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(val_dataloader, desc="Validating"):
                loss, _, corr_probs = get_loss(model, batch, true_token, false_token, args)
                total_val_loss += loss.item()
                val_preds.extend(corr_probs.tolist())
        ...
        val_acc, val_auc, _, _, val_f1 = compute_metrics(val_labels, val_preds)
        
        print(f"Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}")
        print(f"Val Acc: {val_acc:.2f}, Val AUC: {val_auc:.2f}, Val F1: {val_f1:.2f}")
        ...
        save_model = False
        if args.use_best_auc:
            if best_val_auc is None or val_auc > best_val_auc:
                print(f"New best AUC: {val_auc:.2f}")
        ...
        if save_model:
            print("Saving model...")
            model_name = args.model_name + (f"_fold{fold}_irt{use_irt}_lr{args.lr}_r{args.r}" if fold else "")
            model.save_pretrained(get_checkpoint_path(model_name))
            
            # Log best metrics to wandb
            if getattr(args, 'use_wandb', False) and wandb is not None:
                wandb.log({
                    'best_val_loss': best_val_loss if best_val_loss is not None else avg_val_loss,
                    'best_val_auc': best_val_auc if best_val_auc is not None else val_auc,
                })
...
def test_lmkt(args, fold):
    ...
    model_name = args.model_name and args.model_name + (f"_fold{fold}_irt{use_irt}_lr{args.lr}_r{args.r}" if fold else "")
    model, tokenizer = get_model(
    ...
    if use_irt and hasattr(model, 'logit_scale'):
        print(f"[Test] logit_scale = {model.logit_scale.item():.6f}")
    ...
    for batch_idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        # print(f"labels: {batch['labels'].tolist()}, preds: {corr_probs.tolist()}, kcs: {[sample['kcs'] for sample in batch['meta_data']]}")
    ...
    for dia_idx, sample in test_df.iterrows():
        ...
        for turn in dialogue:
            if turn["correct"] is not None and not first_turn:
                ...
                if use_irt:
                    theta_val = f"{all_kc_probs[label_idx][0]:.4f}"
                    diff_val = f"{all_kc_probs[label_idx][1]:.4f}"
                label_counter += 1
            else:
                prob = "--"
                kc_probs = "--"
            ...
            if use_irt:
                row["Theta"] = theta_val
                row["Difficulty"] = diff_val
            qual_data.append(row)
    ...
    if getattr(args, 'use_wandb', False) and wandb is not None:
        wandb_metrics = {
            'test_loss': loss,
            'test_accuracy': all_metrics[0],
            'test_auc': all_metrics[1],
            'test_precision': all_metrics[2],
            'test_recall': all_metrics[3],
            'test_f1': all_metrics[4],
        }
        if final_metrics:
            wandb_metrics.update({
                'test_final_accuracy': final_metrics[0],
                'test_final_auc': final_metrics[1],
                'test_final_precision': final_metrics[2],
                'test_final_recall': final_metrics[3],
                'test_final_f1': final_metrics[4],
            })
