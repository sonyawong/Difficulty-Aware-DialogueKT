...
def load_annotated_data(args, fold: Union[int, str, None] = 1):
    if args.dataset == "comta":
        ...
        need_cv = bool(getattr(args, "crossval", False)) or (fold is not None)
        if need_cv:
            # 确保有种子（若 args.seed 不存在就用 221）
            seed = getattr(args, "seed", 221)
            ...
            K = getattr(args, "n_folds", 5)
            assert isinstance(K, int) and K >= 2
            assert fold is None or (isinstance(fold, int) and 1 <= fold <= K)

            splits = np.array_split(np.arange(len(full)), K)
            # 若 fold 未传（但 crossval=True），就返回默认第 1 折；真正循环时用 crossval() 传入 1..K
            f = 1 if fold is None else fold
            val_idx = splits[f - 1]
            train_idx = np.setdiff1d(np.arange(len(full)), val_idx, assume_unique=False)

            cv_train_df = full.iloc[train_idx].reset_index(drop=True)
            cv_valid_df = full.iloc[val_idx].reset_index(drop=True)
            return cv_train_df, cv_valid_df, test_df
